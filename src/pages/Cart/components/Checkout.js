...
const Checkout = ({setCheckout}) => {
    ...
    // Fetch Api data to have users information
    useEffect(() => {
    ...
    async function handleOrderSubmit(event){
        ...
        // Try catch in case data isn't recieved
       try { 
            // Call to our API 
            const data = await createOrder(cartList, total, user); 
            // clearing cart
            clearCart();
            // navigating and passing data through navigate using state
    ...
    return ( 
        <section className="z-20">
            <div className="fixed z-20 top-0 left-0 w-full h-full bg-black bg-opacity-50"></div>
            <div id="authentication-modal" tabIndex="-1" className="fixed h-full top-0 right-0 z-50 w-full justify-center items-center flex" aria-modal="true" role="dialog" >
                <div className="relative p-4 w-full max-w-md h-full md:h-auto overflow-y-auto">
                    ...
                    <div className="py-6 px-6 lg:px-8">
                        <h3 className="mb-2 text-l font-medium text-gray-900 dark:text-white">
